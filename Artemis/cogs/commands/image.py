    ...
    async def image(self, ctx):
        ...
        def get_response(response):
            ...
            if response.attachments:
                ...
                to_base64 = Artemis.provider.processor.encode(response.attachments[0].url)

                # save bas64 image data to database
                Artemis.provider.set_using_image(ctx.message.guild.id, True)
                Artemis.provider.save_image(ctx.message.guild.id, to_base64)
        ...
        try:
            ...
            await ctx.message.channel.send("Image set!, run command '{}test' to preview".format(Artemis.prefix))
